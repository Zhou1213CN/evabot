...
from backend.app.channels.channel_config import DiscordConfig
...
def split_message(text: str, max_length: int) -> list[str]:
    """简单安全的文本切断工具，防止超出 Discord 的单条消息长度限制"""
    if not text:
        return []
...
class DiscordChannel(BaseChannel):
    ...
    def __init__(self, config: DiscordConfig, gateway: Gateway):
    ...
    async def send(self, msg: Message) -> None:
        ...
        url = f"{DISCORD_API_BASE}/channels/{msg.receiver_id}/messages"
        ...
        try:
            ...
            # Evabot 中的附加参数存放于 data 中
            reply_to = (msg.data or {}).get("reply_to")
    ...
    async def _send_file(
        ...
        path = Path(file_path)
        if not path.is_file():
            logger.warning(f"Discord file not found, skipping: {file_path}")
            return False

        if path.stat().st_size > MAX_ATTACHMENT_BYTES:
            logger.warning(f"Discord file too large (>20MB), skipping: {path.name}")
            return False
    ...
    async def _identify(self) -> None:
        ...
        # Fallback to a default intents integer if not specified (e.g. 33280 matches standard message intents)
        intents_val = getattr(self.config, "intents", 33280)
        ...
        if group_policy == "mention":
            if self._bot_user_id:
                ...
                if f"<@{self._bot_user_id}>" in content or f"<@!{self._bot_user_id}>" in content:
                    return True
            logger.debug(f"Discord message in {payload.get('channel_id')} ignored (bot not mentioned)")
            return False
