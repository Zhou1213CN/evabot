...
from backend.app.channels.channel_config import MochatConfig

logger = get_logger("channels.mochat")
...
# --- [Evabot 架构适配] 运行时状态目录隔离 ---
def get_runtime_subdir(name: str) -> Path:
    """获取运行时状态持久化目录"""
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../workspace/runtime'))
...
def resolve_require_mention(config: MochatConfig, session_id: str, group_id: str) -> bool:
    ...
    for key in (group_id, session_id, "*"):
        if key and key in groups:
            return bool(getattr(groups[key], "require_mention", False))
    return bool(getattr(getattr(config, "mention", object()), "require_in_groups", False))
...
class MochatChannel(BaseChannel):
    ...
    def __init__(self, config: MochatConfig, gateway: Gateway):
    ...
    async def start(self) -> None:
        ...
        if not getattr(self.config, 'claw_token', None):
    ...
    async def send(self, msg: Message) -> None:
        ...
        # 从 Evabot Artifacts 获取待发送媒体路径
        media_paths = [art.uri for art in msg.artifacts] if msg.artifacts else []
        if media_paths:
            parts.extend(m for m in media_paths if isinstance(m, str) and m.strip())
            
        content = "\n".join(parts).strip()
        if not content:
            return

        target = resolve_mochat_target(msg.receiver_id)
        if not target.id:
        ...
        try:
            if is_panel:
                await self._api_send("/api/claw/groups/panels/send", "panelId", target.id,
                                     content, reply_to, self._read_group_id(msg_data))
            else:
    ...
    def _seed_targets_from_config(self) -> None:
        sessions, self._auto_discover_sessions = self._normalize_id_list(getattr(self.config, 'sessions', []))
        panels, self._auto_discover_panels = self._normalize_id_list(getattr(self.config, 'panels', []))
    ...
    async def _start_socket_client(self) -> bool:
        ...
        serializer = "default"
        if not getattr(self.config, 'socket_disable_msgpack', False):
        ...
        client = socketio.AsyncClient(
            reconnection=True,
            reconnection_attempts=getattr(self.config, 'max_retry_attempts', None),
            reconnection_delay=max(0.1, getattr(self.config, 'socket_reconnect_delay_ms', 1000) / 1000.0),
            reconnection_delay_max=max(0.1, getattr(self.config, 'socket_max_reconnect_delay_ms', 5000) / 1000.0),
        ...
        socket_url = (getattr(self.config, 'socket_url', '') or base_url).strip().rstrip("/")
        socket_path = (getattr(self.config, 'socket_path', '/socket.io')).strip().lstrip("/")
        ...
        try:
            ...
            await client.connect(
                ...
                wait_timeout=max(1.0, getattr(self.config, 'socket_connect_timeout_ms', 20000) / 1000.0),
            )
            return True
        except Exception as e:
    ...
    async def _subscribe_sessions(self, session_ids: list[str]) -> bool:
        ...
        ack = await self._socket_call("com.claw.im.subscribeSessions", {
            ...
            "limit": getattr(self.config, 'watch_limit', 50),
        })
    ...
    async def _refresh_loop(self) -> None:
        interval_s = max(1.0, getattr(self.config, 'refresh_interval_ms', 60000) / 1000.0)
        while self._running:
        ...
        while self._running and self._fallback_mode:
            try:
                payload = await self._post_json("/api/claw/sessions/watch", {
                    ...
                    "timeoutMs": getattr(self.config, 'watch_timeout_ms', 30000), 
                    "limit": getattr(self.config, 'watch_limit', 50),
                })
                await self._handle_watch_payload(payload, "session")
                ...
                await asyncio.sleep(max(0.1, getattr(self.config, 'retry_delay_ms', 5000) / 1000.0))

    async def _panel_poll_worker(self, panel_id: str) -> None:
        sleep_s = max(1.0, getattr(self.config, 'refresh_interval_ms', 60000) / 1000.0)
        while self._running and self._fallback_mode:
            try:
                resp = await self._post_json("/api/claw/groups/panels/messages", {
                    "panelId": panel_id, "limit": min(100, max(1, getattr(self.config, 'watch_limit', 50))),
                })
    ...
    async def _process_inbound_event(self, target_id: str, event: dict[str, Any], target_kind: str) -> None:
        ...
        if not author or (getattr(self.config, 'agent_user_id', None) and author == self.config.agent_user_id):
            return
        ...
        agent_user_id = getattr(self.config, 'agent_user_id', None)
        was_mentioned = resolve_was_mentioned(payload, agent_user_id) if agent_user_id else False
        require_mention = target_kind == "panel" and is_group and resolve_require_mention(self.config, target_id, group_id)
        use_delay = target_kind == "panel" and getattr(self.config, 'reply_delay_mode', None) == "non-mention"
    ...
    async def _delay_flush_after(self, key: str, target_id: str, target_kind: str) -> None:
        await asyncio.sleep(max(0, getattr(self.config, 'reply_delay_ms', 0)) / 1000.0)
    ...
    async def _post_json(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self._http.post(url, headers={
            "Content-Type": "application/json", "X-Claw-Token": getattr(self.config, 'claw_token', ''),
        }, json=payload)
