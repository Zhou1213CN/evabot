...
from backend.app.channels.channel_config import WeixinConfig

logger = get_logger("channels.weixin")
...
class WeixinChannel(BaseChannel):
    ...
    def __init__(self, config: WeixinConfig, gateway: Gateway):
    ...
    async def _poll_once(self) -> None:
        ...
        if is_error:
            if errcode == ERRCODE_SESSION_EXPIRED or ret == ERRCODE_SESSION_EXPIRED:
                ...
                remaining = self._session_pause_remaining_s()
                logger.warning(
                    f"WeChat session expired (errcode {errcode}). Pausing {max((remaining + 59) // 60, 1)} min."
                )
                return
            raise RuntimeError(
    ...
    async def _process_message(self, msg: dict) -> None:
        ...
        # 调用 base.py 中的 _handle_message，推入 Gateway
        ...
        try:
            ...
            for idx, (download_source, cdn_url) in enumerate(download_candidates):
                try:
                    ...
                    if should_fallback:
                        logger.warning(
                            f"WeChat media download failed via full_url, falling back to encrypt_query_param: type={media_type} err={e}"
                        )
                        continue
                    raise
    ...
    async def _send_media_file(
        ...
        if not download_param:
            raise RuntimeError(
                f"CDN upload response missing x-encrypted-param header; status={cdn_resp.status_code} headers={dict(cdn_resp.headers)}"
            )
